...
def P(xval,sigc,gamc,sigt,gamt,N,E,c0,c1,beta):		#psf 	x=dRA, y=dDEC	umrechnung in rad noetig
	res=f(N,sigt,sigc)*k(xval,sigc,gamc,E,c0,c1,beta)+(1.-f(N,sigt,sigc))*k(xval,sigt,gamt,E,c0,c1,beta)
	return res
...
#drad=np.arange(0.9,0.9999999,0.0001)
drad=np.linspace(0.,0.25*pi/180.,100)
for psft in np.arange(0,4,1):
    norma = np.zeros((8,23))
    pind = np.int_(psft*3+1)
    ...
    for i in np.arange(0,8,1):
     for j in np.arange(0,23,1):
         ...
         norma[i,j]=simps(prad,drad)
         print norma[i,j]

    plt.imshow(norma,interpolation='nearest')
    plt.colorbar()
    plt.show()
...
drad= np.linspace(0,100.,1000)
resu=[]
for i in np.arange(10,elist.size):
    prad=(np.asarray(map(lambda x: twopi*x*P(x,sigc[i],gamc[i],sigt[i],gamt[i],N[i],elist[i],c0[i],c1[i],beta[i]),drad)))
    print elist[i],(simps(prad,drad))


print 'XX'
drad = np.linspace(0.,45.*pi/180.,10000)
resu=[]
for i in np.arange(10,elist.size):
